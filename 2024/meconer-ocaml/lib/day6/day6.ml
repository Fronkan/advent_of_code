...
let grid = (List.map ~f:String.to_list day6Input)|> List.fold ~init:[] ~f:(@) |> Array.of_list 
...
let getStateOfPos pos = 
  ...
  if ( pos.col < 0 || pos.col >= width || pos.row < 0 || pos.row >= height ) then OffGrid
  else
...
let rec moveOneStep pos direction =
  let nextPos = 
    ...
    | West -> {pos with col = pos.col - 1} in
...
let startPos = fst (Array.findi_exn grid ~f:( fun _idx c -> Char.equal c '^' ) ) |> idxToCoord
...
let rec doMove  accum pos direction=
  ...
  | OffGrid -> accum
  | Empty | Obstacle -> doMove (Set.add accum (string_of_pos nextPos)) nextPos nextDir
...
let rec moveOneStepWithObstacle pos direction obstaclePos =
  ...
  let state = if posEqual nextPos obstaclePos then Obstacle
  else getStateOfPos nextPos in
...
let rec doMoveWithObstacle accum pos direction obstaclePos =
  let nextPos, nextDir, nextState = moveOneStepWithObstacle pos direction obstaclePos in
  (* Printf.printf "%s\n" (string_of_pos_and_dir nextPos nextDir); *)
